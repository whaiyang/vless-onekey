...
build_vless_url() {
  ENCODED_NODE_NAME="$(python3 - <<PY
import urllib.parse
print(urllib.parse.quote("${NODE_NAME}", safe=""))
PY
)"

  VLESS_URL="vless://${UUID}@${PUBLIC_IP}:${PORT}?encryption=none&type=tcp&security=reality&sni=${SNI}&fp=${CLIENT_FINGERPRINT}&flow=xtls-rprx-vision&pbk=${PUBLIC_KEY}&sid=${SHORT_ID}#${ENCODED_NODE_NAME}"
}
